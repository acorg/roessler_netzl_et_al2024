...
full_map <-read.acmap("./data/maps/map_threshold20_all_ags_singleTP_woXBBBQ11conv_alpha_adj.ace")
...
png(file.path(figure_dir, "proc_map_threshold20_all_ags_singleTP_woXBBconvStudy_alpha_adj.png"), width = 12, height = 5, units = 'in', res=300, pointsize = 18)
...
png(file.path(figure_dir, "proc_map_threshold20_all_ags_singleTP_woXBBconvStudy_alpha_adj_show_only_commonAgs.png"), width = 12, height = 5, units = 'in', res=300, pointsize = 18)
layout(matrix(c(1:2), ncol = 2, byrow = T))
par(mar = rep(0.5, 4))
...
text(xlim_no_zoom[1]+0.4, ylim_no_zoom[2]-0.4, "A", cex = 1.2)
text(xlim_no_zoom[2]-0.2, ylim_no_zoom[1]+ 0.2, "Human data set 1", cex = 0.8, adj = c(1, 0), col = "grey30")
doplot(procrustesMap(move_coords(removeAntigens(duke_map, agNames(duke_map)[!agNames(duke_map) %in% agNames(full_map)]), at = 1, by = -3), full_map, scaling = TRUE), xlim_no_zoom-3, ylim_no_zoom, FALSE)
text(xlim_no_zoom[1]-3+0.4, ylim_no_zoom[2]-0.4, "B", cex = 1.2)
text(xlim_no_zoom[2]-3- 0.2, ylim_no_zoom[1] + 0.2, "Human data set 2", cex = 0.8, adj = c(1, 0), col = "grey30")
dev.off()
...
png(file.path(figure_dir, "proc_map_threshold20_all_ags_singleTP_woXBBconvStudy_alpha_adj_unscaled.png"), width = 12, height = 5, units = 'in', res=300, pointsize = 18)
layout(matrix(c(1:2), ncol = 2, byrow = T))
par(mar = rep(0.5, 4))
