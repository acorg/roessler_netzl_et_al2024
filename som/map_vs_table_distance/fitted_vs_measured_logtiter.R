...
plot_map_vs_table_titers <- function(map) {
  ...
  ggplot() +
    ...
    labs(title = paste0(capitalize(gsub(" conv.| vax.", "", sr)), " sera")) +
...
map_ns <- c("map_threshold20_all_ags_singleTP_woXBBBQ11conv_alpha_adj.ace")
...
for(map_n in map_ns){
  ...
  ggsave(filename = paste0("som/map_vs_table_distance/", gsub(".ace", "_mapVstable.png", map_n)), plot = combo, width=10, height=12, dpi = 300)
  
  ggpubr::ggarrange(titers_final, titers_res_final, nrow = 2) -> combo
  ggsave(filename = paste0("som/map_vs_table_distance/", gsub(".ace", "_mapVstable2.png", map_n)), plot = combo, width=5, height=4, dpi = 300)
  
  
}


ggpubr::ggarrange(titers_top, titers_final, widths = c(0.5), nrow = 2)
